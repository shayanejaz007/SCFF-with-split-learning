...
Figure~\ref{fig:single_console} shows the console output for the 10-client relay configuration
(10 clients, 1 server), confirming final numbers of 92.10\% train accuracy and 79.77\% test
accuracy. The gap between raw training accuracy and the readout train accuracy is expected; the
linear probe overfits somewhat on the full training set, while the reported test accuracy remains
...
\begin{figure}[htbp]
  ...
  \caption{Console output for the 10-client relay run (10 clients, 1 server). Final train
           accuracy: 92.10\%; final test accuracy: 79.77\%.}
