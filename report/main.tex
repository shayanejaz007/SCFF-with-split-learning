...
configuration reaches 92.10\% training accuracy and 79.77\% test accuracy on the linear readout.
...
\begin{figure}[htbp]
  ...
  \caption{Linear readout accuracy over 50 epochs for the single-client split configuration.
           ...
           $\approx$85\%; test accuracy plateaus near 80\%.}
  \label{fig:single_accuracy}
\end{figure}

The console output (Figure~\ref{fig:single_console}) confirms the final numbers: train accuracy
of 92.10\% and test accuracy of 79.77\%. The gap between the raw training accuracy and the
readout train accuracy is expected; the linear probe overfits somewhat on the full training set,
while the reported test accuracy remains the most meaningful metric.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{images/fig2_1client_console.jpeg}
  \caption{Console output for the single-client split run. Final train accuracy: 92.10\%;
